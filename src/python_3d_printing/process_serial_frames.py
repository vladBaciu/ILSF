...
class SerialFrame:
    ...
    def __init__(self, serialCOM, baudRate):
            self.serialHandler = serial.Serial(port=serialCOM, baudrate=baudRate, parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS)
            if self.serialHandler.isOpen() == False:
